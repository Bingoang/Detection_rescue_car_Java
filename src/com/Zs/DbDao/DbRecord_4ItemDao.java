...
public class DbRecord_4ItemDao {
	...
	public DefaultListModel getAll(Connection con) throws SQLException
	{
		...
		while(rs.next()){
			m.addElement(rs.getString("Humidity") + 
						 rs.getString("Temperature")+
						 rs.getString("Barrier")+
						 rs.getString("RunningDistance")+
						 rs.getString("Live")+
	...
	{
		DefaultListModel m = new DefaultListModel();
		String sql="select * from record where DoSth=?";
		PreparedStatement pstmt=con.prepareStatement(sql);//预编译
	...
	{
		DefaultListModel m = new DefaultListModel();
		String sql="select * from record where UseSth=?";
		PreparedStatement pstmt=con.prepareStatement(sql);//预编译
	...
	{
		DefaultListModel m = new DefaultListModel();
		String sql="select * from record where "+item+"=?";
		PreparedStatement pstmt=con.prepareStatement(sql);//预编译
	...
	public DefaultListModel getSearchingDb_(Connection con,String item) throws SQLException
	{
		DefaultListModel m = new DefaultListModel();
		String sql="select * from record where "+item+"<>''";
		PreparedStatement pstmt=con.prepareStatement(sql);//预编译
...
//		pstmt.setString(1, content);
	...
	public static  int dbRecord_4ItemAdd(Connection con,DbRecord_4ItemMemb record) throws Exception{
		String sql="insert into record values(?,?,?,?,?,?,null)";//操作的ID也是自增的
		//括号里表示用户表从0~n里的属性。
		PreparedStatement pstmt=con.prepareStatement(sql);
		pstmt.setString(1,record.getHumidity());
		pstmt.setString(2, record.getTemperature());
		pstmt.setString(3, record.getBarrier());
		pstmt.setString(4, record.getRunningDistance());
		pstmt.setString(5, record.getLive());
		pstmt.setString(6, ZsUtil.DateToSqlDate_String(TestDate.getDate()));//getInStime().toString()将日期转化成字符串
